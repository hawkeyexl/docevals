...
- `src/core/resolve.ts` — merges page frontmatter (the `evals` key: array shorthand or object form with suite/skip) with `docevals.config.yaml` suites/named evals. Page wins on name collision. `type` defaults to `regression`, `grader` to `llm`.
